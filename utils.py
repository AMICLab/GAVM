...
def save_imgs(img, msk, msk_pred, save_path, batch_idx):
    # 移除多余的维度
    img = img.squeeze()
    msk = msk.squeeze()
    ...
    msk = msk * normalization_factor
    msk_pred = msk_pred * normalization_factor
    # 确保保存路径存在
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    def get_filename(type_name):
        return os.path.join(save_path, f"{type_name}_{batch_idx}.npy")
    np.save(get_filename("img"), img)

    np.save(get_filename("msk"), msk)

    np.save(get_filename("pred"), msk_pred)
...
def cal_params_flops(model, size, logger):
    input = torch.randn(1, 1, size, size).cuda()
